...
public class Application {
	...
	public static void setCustomerController(AuthorController customerController) {
	...
	public static void setEmployeeController(AssistantEditorController employeeController) {
	...
	public static void initControllers(String userType) {
		...
		switch (userType.toUpperCase()) {
			case "CUSTOMER": 
				Application.customerController = AuthorController.getCustomerController();  
				System.out.printf("Welcome, %s! A truly-valued customer!\n\n", 
						Application.getCurrentUser().getUsername() );
				break; 
			case "EMPLOYEE": 
				Application.employeeController = AssistantEditorController.getEmployeeController(); 
				System.out.printf("Welcome, %s! Our shop doesn't work if you don't!\n\n", 
						Application.getCurrentUser().getUsername() );
				break; 
			case "MANAGER": 
				Application.managerController = SeniorEditorrController.getManagerController(); 
	...
	static void getUserResponse() {
		...
		case "CUSTOMER": 
			AuthorController.dispatchUserActionBasedOnResponse(userResponse);
			break; 
		case "EMPLOYEE":
			AssistantEditorController.dispatchUserActionBasedOnResponse(userResponse);
			break;
		case "MANAGER":
			SeniorEditorrController.dispatchUserActionBasedOnResponse(userResponse);
			break; 
		}
	}
