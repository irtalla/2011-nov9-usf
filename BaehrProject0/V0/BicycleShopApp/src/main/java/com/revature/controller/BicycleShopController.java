...
public class BicycleShopController {
	...
	private static OfferServices oServ = new OfferServicesImpl();
	private static PurchaseServices pServ = new PurchaseServicesImpl();
	private static BicycleServices bServ = new BicycleServicesImpl();
	private static BicycleTypeServices btServ = new BicycleTypeServicesImpl();
	...
	private static User viewPurchases(User loggedInUser) {
		Set<Purchase> purchases = pServ.getAll();
		...
		while (true) {
			...
			try {
				...
				case 2:
					...
					outstandingOffers = oServ.getOutstanding();
				...
				case 3:
					...
					outstandingOfferset = oServ.getOutstanding();
					if(outstandingOfferset.size() > 0) {
						...
						try {
							int userinput = Integer.valueOf(scan.nextLine());
							Offer o = oServ.getOfferByOfferId(userinput);
							if(o == null) {
	...
	private static User registerUser() {
		while (true) {
			...
			try {
				...
				case 1:
					try {
						newAccount.setUserID((uServ.addUser(newAccount))); 
	...
	private static void viewAvailableBicycles(User user) {
		Set<Bicycle> availableBicycles = bServ.getAvailable();
		...
		try {
			...
			if (input == 1) {
				while (true) {
					...
					Bicycle b = bServ.getBicycleById(input);
					if (b != null && b.getStatus().equals("Available")) {
						...
						if (input == 1) {
							...
							if (input == 1) {
								...
								//System.out.println(o);
								Offer updatedOffer = oServ.getOfferByOfferId(newId);
	...
	private static void viewUserBicycles(User user) {
		...
		userBikes = bServ.getByUserId(user.getUserID());
		if (userBikes.size() > 0) {
		...
		while (true) {
			...
			try {
				...
				if (input == 1) {
					...
					bt.setId(btServ.addBicycleType(bt));
					...
					input = Integer.valueOf(scan.nextLine());
					if (input == 1) {
						newBicycle.setId(bServ.addBicycle(newBicycle));
						System.out.println("You successfully added the following bicycle: \n" + newBicycle);
					}
				} else if (input == 2) {
					for (Bicycle b : bServ.getAvailable()) {
						System.out.println(b);
					}
					System.out.println("Which bicycle would you like to update? Enter its ID.");
					Bicycle b = bServ.getBicycleById(Integer.valueOf(scan.nextLine()));
					Bicycle newBicycle = new Bicycle();
							...
							try {
								...
								case 1:
									...
									if(typeId.equals(1) || typeId.equals(2) || typeId.equals(3)) {
										b.setBicycleType(btServ.getById(typeId));
									}
									else {
			...
			try {
				...
				if (input == 1) {
					...
					if (input == 1) {
						...
						personToRemove = uServ.getUserById(Integer.valueOf(scan.nextLine()));
					} else if (input == 2) {
						System.out.println("Enter the username of the user you want to remove.");
						personToRemove = uServ.getUserByUsername(scan.nextLine());
					}
					if (input == 1 || input == 2) {
						if (personToRemove != null) {
							...
							input = Integer.valueOf(scan.nextLine());
							if (input == 1) {
								uServ.deleteUser(personToRemove);
