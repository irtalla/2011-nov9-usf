...
public Usr getById(Integer id) {
	...
	try (Connection conn = cu.getConnection()) {
		String sql = "select usr.id as id, usr_role.id as usr_role_id, username, passwd, "
...
public void delete(Usr t) {
	try (Connection conn = cu.getConnection()) {
		...
		sql = "delete from user_bike where user_id = ?";
		pstmt.setInt(1, t.getId());
...
public Usr getByUsername(String username) {
	...
	try (Connection conn = cu.getConnection())
			{
				String sql = "select usr.id as id, usr.usr_role_id, usr.username, usr.passwd, usr_role.name from usr full join usr_role on usr_role.id = usr_role_id where username = ?";
				PreparedStatement pstmt = conn.prepareStatement(sql);
...
public void accept(Integer id) {
	try (Connection conn = cu.getConnection()) {
		...
		String sql2 = "update offers set accept_status = 'Accepted' where offerer = ? AND offer_number = ?";
		...
		int bikeid = offer.getTargetBike().getId();
			...
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, offer.getTargetBike().getId());
			pstmt.executeUpdate();
			conn.commit();
